# Module: News Feed
The `newsfeed ` module is one of the default modules of the XMirora.
...
XMirora's [notification mechanism](https://gitHub.com/mohan-pannirselvam/XMirora/tree/master/modules#thissendnotificationnotification-payload) allows to send notifications to the `newsfeed` module. The following notifications are supported:
...
#### Example
The following example shows how the next news article title can be displayed on the XMirora.
````javascript
this.sendNotification('ARTICLE_NEXT');
````

#### `newsfeed` specific notification emitting modules
The third party [MMM-Gestures](https://github.com/thobach/MMM-Gestures) module supports above notifications when moving your hand up, down, left or right in front of a gesture sensor attached to the XMirora. See module's readme for more details.
