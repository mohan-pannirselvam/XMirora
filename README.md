...
*Electron*, the app wrapper around XMirora², only supports the Raspberry Pi 2/3. The Raspberry Pi 0/1 is currently **not** supported. If you want to run this on a Raspberry Pi 1, use the [server only](#server-only) feature and setup a fullscreen browser yourself. (Yes, people have managed to run MM² also on a Pi0, so if you insist, search in the forums.)
...
2. Clone the repository and check out the master branch: `git clone https://gitHub.com/mohan-pannirselvam/XMirora`
3. Enter the repository: `cd XMirora/`
...
In some cases, you want to start the application without an actual app window. In this case, you can start XMirora² in server only mode by manually running `node serveronly` or using Docker. This will start the server, after which you can open the application in your browser of choice. Detailed description below.
...
XMirora² in server only mode can be deployed using [Docker](https://docker.com). After a successful [Docker installation](https://docs.docker.com/engine/installation/) you just need to execute the following command in the shell:
...
docker run  -d \
	...
	--name magic_mirror \
	bastilimbach/docker-XMirora
```
To get more information about the available Dockerfile versions and configurations head over to the respective [GitHub repository](https://github.com/bastilimbach/docker-XMirora).
...
The following wiki links are helpful for the initial configuration of your XMirora² operating system:
- [Configuring the Raspberry Pi](https://gitHub.com/mohan-pannirselvam/XMirora/wiki/Configuring-the-Raspberry-Pi)
- [Auto Starting XMirora](https://gitHub.com/mohan-pannirselvam/XMirora/wiki/Auto-Starting-XMirora)


### General

1. Copy `/home/pi/XMirora/config/config.js.sample` to `/home/pi/XMirora/config/config.js`. \
   **Note:** If you used the installer script. This step is already done for you.

2. Modify your required settings. \
   Note: You'll can check your configuration running `npm run config:check` in `/home/pi/XMirora`.
...
| **Option** | **Description** |
| --- | --- |
| `port` | The port on which the XMirora² server will run on. The default value is `8080`. |
...
| `ipWhitelist` | The list of IPs from which you are allowed to access the XMirora². The default value is `["127.0.0.1", "::ffff:127.0.0.1", "::1"]`, which is from `localhost` only. Add your IP when needed. You can also specify IP ranges with subnet masks (`["127.0.0.1", "127.0.0.1/24"]`) or directly with (`["127.0.0.1", ["192.168.0.1", "192.168.0.100"]]`). Set `[]` to allow all IP addresses. For more information see: [follow post ipWhitelist HowTo](https://forum.XMirora.builders/topic/1326/ipwhitelist-howto) |
...
For more available modules, check out out the wiki page [XMirora² 3rd Party Modules](https://gitHub.com/mohan-pannirselvam/XMirora/wiki/3rd-party-modules). If you want to build your own modules, check out the [XMirora² Module Development Documentation](modules) and don't forget to add it to the wiki and the [forum](https://forum.XMirora.builders/category/7/showcase)!


## Updating

If you want to update your XMirora² to the latest version, use your terminal to go to your Magic Mirror folder and type the following command:
...
The community around the XMirora² is constantly growing. We even have a [forum](https://forum.XMirora.builders) now where you can share your ideas, ask questions, help others and get inspired by other builders. We would love to see you there!
...
Thanks for your help in making XMirora² better!

## Manifesto

A real Manifesto is still to be written. Till then, Michael's response on [one of the repository issues](https://gitHub.com/mohan-pannirselvam/XMirora/issues/1174) gives a great summary:

> "... I started this project as an ultimate starter project for Raspberry Pi enthusiasts. As a matter of fact, for most of the contributors, the XMirora project is the first open source project they ever contributed to. This is one of the reasons why the XMirora project is featured in several RasPi magazines.
>
...
<p align="center">
<br>
	<a href="https://forum.XMirora.builders/topic/728/XMirora-is-voted-number-1-in-the-magpi-top-50"><img src="https://XMirora.builders/img/magpi-best-watermark-custom.png" width="150" alt="MagPi Top 50"></a>
</p>
